 ...
 * @param  	{String}  dest			Destination folder for the TypeHelper.cs file
 * @param  	{Array}   native_types  Native types used in the app
 * @param  	{Array}   native_events Native events used in the app
 ...
 * @param  {String}   dest         Destination folder to write the RequireHook.cpp file
 * @param  {Array}   modules      Modules to use in the app
 * @param  {Array}   native_types Native types used in the app
 * @param  {Function} next         Callback to call once finished
 */
function generateRequireHook(dest, modules, native_types, next) {
	...
	fs.readFile(template, 'utf8', function (err, data) {
		...
		let native_module_includes = [], // built up includes
...
function runNuGet(slnFile, callback) {
	...
	let nuget = path.resolve(__dirname, '..', 'vendor', 'nuget', 'nuget.exe'),
...
function runMSBuild(slnFile, buildConfiguration, callback) {
	...
	let p = spawn((process.env.comspec || 'cmd.exe'), [ '/S', '/C', '"', vsInfo.vsDevCmd.replace(/[ ()&]/g, '^$&')
...
exports.generate = function generate(builder, finished) {
	...
	let sdkVersion = builder.targetPlatformSdkVersion === '10.0' ? windowsInfo.windows['10.0'].sdks[0] : builder.targetPlatformSdkVersion,
